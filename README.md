...
{
  "$schema": "https://opencode.ai/config.json",
  "plugin": ["@toady00/open-mardi-gras@1.3.0"]
}
```

- `@toady00/open-mardi-gras@1.3.0` - exact pin, best cache behavior
- `@toady00/open-mardi-gras` - always track latest
- Semver ranges like `@toady00/open-mardi-gras@^1.3.0` work, but may reinstall on every startup due to OpenCode's current cache behavior
