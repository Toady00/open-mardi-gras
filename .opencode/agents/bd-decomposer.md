...
Load the `beads-commands` and `beads-epics` skills before creating any beads.
...
description tells the work agent to invoke `@bd-reviewer`. The reviewer files
findings as beads with `discovered-from` links. The review bead is closed only
when the review is complete.

### Epic and spec relationship
Epics are created during spec writing (`/bd-spec`) or tracking (`/bd-spec-track`).
